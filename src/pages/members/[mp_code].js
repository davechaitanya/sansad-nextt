...
import { Spinner, ErrorBox, EmptyState, Pagination } from "../../components/UI";
...
export default function MemberProfilePage({ member: initialMember, dashboard: initialDashboard }) {
  ...
  useEffect(() => {
    ...
    Promise.all([api.getMemberProfile(mp_code), api.getDashboard(mp_code).catch(()=>null)])
      .then(([profile, dash]) => {
        setMember(profile?.member || profile);
        setDashboard(dash);
      })
  ...
  return (
    <>
      <Head>
        <title>{F.name(member)} — Parliament of India</title>
        <meta name="description" content={`Profile of ${F.name(member)}, MP from ${F.constituency(member)}, ${F.party(member)}`} />
      </Head>
      <div className="fade-in">
        <div style={{ maxWidth:1200, margin:"0 auto", padding:"32px 24px 0" }}>
          <Link href="/members" style={{ fontSize:13, color:"#e8651a", textDecoration:"none", fontFamily:"'DM Sans',sans-serif", display:"inline-flex", alignItems:"center", gap:6, marginBottom:24 }}>
            ← Back to Members
          </Link>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(300px, 1fr))", gap:20 }}>
      ...
      <div className="card" style={{ padding:24 }}>
        ...
        {[
          ...
          ["Terms Served", member.terms],
          ["MP Code", member.mp_code],
        ].filter(([,v]) => v).map(([k,v]) => (
      ...
      {personal && (
        <div className="card" style={{ padding:24 }}>
          ...
          {[
            ["Date of Birth", personal.dateBirth],
            ["Place of Birth", personal.placeBirth],
            ...
            ["Sons", personal.no_of_sons ?? personal.noSons],
            ["Daughters", personal.no_of_daughters ?? personal.noDaughters],
        ...
        <tbody>
          {data.map(q => (
            <tr key={q.questionId}>
              ...
              <td style={{ maxWidth:300 }}><div style={{ overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{q.subject}</div></td>
              <td style={{ fontSize:12, color:"#666" }}>{q.ministry}</td>
              <td><span style={{ fontSize:11, padding:"2px 8px", borderRadius:6, background:q.questionType==="STARRED"?"#fff0e6":"#f5f3f0", color:q.questionType==="STARRED"?"#e8651a":"#888", fontWeight:600 }}>{q.questionType}</span></td>
        ...
        <div key={c.id} className="card" style={{ padding:"16px 20px" }}>
          <div style={{ display:"flex", gap:10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:12, color:"#e8651a", fontWeight:600, marginBottom:4 }}>{c.status}</div>
...
export async function getServerSideProps({ params }) {
  try {
    const [profileRes, dashRes] = await Promise.all([
      fetch(`https://davechaitanya-loksabha-api.hf.space/api/member-profile/${params.mp_code}`),
      fetch(`https://davechaitanya-loksabha-api.hf.space/api/dashboard/${params.mp_code}`),
    ]);
